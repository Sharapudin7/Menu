...
export default new Vuex.Store({
  actions: {
    async fetchMenu(ctx) {
      const res = await fetch("https://it-dag.ru/restapi/food?restaurant_id=1");
      const menu = await res.json();

      ctx.commit("updateMenu", menu);
    },
    async fetchCategories(ctx) {
      const res = await fetch("https://it-dag.ru/restapi/restaurant?id=1");
  ...
  getters: {
    ...
    searchItems: state => {
      return state.menu.food ? Object.values(state.menu.food).filter(item => {
